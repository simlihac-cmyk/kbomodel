...
function parsePcode(html: string) {
  const match = html.match(/situationscount\.aspx\?pcode=(\d+)/i);
...
export function parseOfficialEnPlayerSituationsCountPitcher(html: string) {
  const $ = loadHtml(html);
  const pcode = parsePcode(html);
  const profile = parseProfileMap($);
  const teamName = parseTeamName($);

  return $("div.tbl_common table[summary='count'] tbody tr")
    .toArray()
    .map((rowElement) => {
