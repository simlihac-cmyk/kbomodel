...
function parsePcode(html: string) {
  const match = html.match(/situationsbatter\.aspx\?pcode=(\d+)/i);
...
export function parseOfficialEnPlayerSituationsPitcher(html: string) {
  const $ = loadHtml(html);
  const pcode = parsePcode(html);
  const profile = parseProfileMap($);
  const teamName = parseTeamName($);

  return $("div.tbl_common table[summary='batter'] tbody tr")
    .toArray()
    .map((rowElement) => {
