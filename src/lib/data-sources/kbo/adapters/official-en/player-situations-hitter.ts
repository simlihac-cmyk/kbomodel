...
function parsePcode(html: string) {
  const match = html.match(/situationspitcher\.aspx\?pcode=(\d+)/i);
...
export function parseOfficialEnPlayerSituationsHitter(html: string) {
  const $ = loadHtml(html);
  const pcode = parsePcode(html);
  const profile = parseProfileMap($);
  const teamName = parseTeamName($);

  return $("div.tbl_common table[summary='pitcher'] tbody tr")
    .toArray()
    .map((rowElement) => {
