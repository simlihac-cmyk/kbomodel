    ...
    <div className="overflow-x-auto">
      <table className="min-w-full border-separate border-spacing-1">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.label}>
              <td className="whitespace-nowrap px-2 py-2 text-sm font-medium text-ink">{row.label}</td>
              {row.values.map((value, index) => (
                <td key={index} className="px-1 py-1">
                  <div
                    className={cn(
                      "flex min-w-12 items-center justify-center rounded-xl px-2 py-2 text-xs font-medium text-ink",
                    )}
                    style={{
